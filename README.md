# MyCardGame
Unity로 제작한 덱빌딩 기반의 카드 전투게임

# 프로젝트 소개
플레이어는 다양한 카드를 사용하여 적을 처치하며 스테이지를 진행
전투를 이기면 카드 보상을 획득하고 덱에 추가하여 보스를 처치하는 것이 목표.
ScriptObject를 활용하여 데이터 관리

## 기능
전투 시스템
- 턴제 전투
- 플레이어 HP
- 적 HP
- 적 반격
- 승리/패배 판정

카드 시스템
- 공격 카드
- 강공격 카드
- 회복 카드
- 방어 카드

덱 시스템
- 덱
- 손
- 묘지
- 셔플
- 카드 강화
- 카드 제거

적 시스템
- 슬라임
- 고블린
- 오크
- 연속 전투

방어 시스템
- 방어도 획득
- 피해 감소

보상 시스템
- 적 처치후 보상 3장 표시
- 보상 카드 1장 선택
- 선택한 카드 묘지에 추가
- 선택 중 전투 카드 사용 불가

스테이지 진행 시스템
- 보상 획득 후 다음 스테이지 진행 표시
- 선택 시 다음 몬스터 등장

코드 리팩토링
- DrawCard/DisCardHand 역할 분리
- 보상 선택, 스테이지 진행 2번에 걸쳐 드로우를 해서
  스테이지 진행 시에만 드로우로 변경

보스 패턴 시스템
- EnemyData 기반 보스 여부
- 보스 특수 공격 데미지 설정
- 특수 공격 데미지 주기 설정
- 노말 적과 보스 행동 분리
- 특수 공격 데미지 랜덤성

HP 시스템
- 플레이어 HP Bar
- 적 HP Bar
- 실시간 체력 UI 갱신
- 적 최대 체력 기반 설정

카드 설명 시스템
- 카드 설명 생성
- 효과 UI
- 카드 데이터와 연동

카드 제거 시스템
- 카드 제거 버튼 추가
- 제거 가능한 카드 3장 표시
- 플레이어가 제거 카드 선택
- 제거 후 전체 카드 수 UI 갱신
- 카드 제거 로그 출력

카드 보상 선택 시스템
- 카드 강화 또는 카드 제거 중 하나만 선택 가능
- 선택 후 다른 관리 행동 비활성화
- 선택하지 않고 다음 스테이지 진행 가능

이벤트 시스템
- Hp회복 이벤트
- 카드 강화 이벤트
- 카드 제거 이벤트
- 구상도
- > 전투 승리
  > 카드 보상 선택
  > 이벤트 선택
  > 이벤트 처리
  > 다음 스테이지 진행
- 이벤트 중 전투 카드 비활성화
- 기존 강화, 제거 시스템 사용
- 이벤트 완료 후 스테이지 버튼 표시

이벤트 확률 시스템
- 전투 보상 후 이벤트가 항상 등장하지 않고, 확률에 따라 등장하도록 구현
- 확률 50% 로 설정

이벤트 구조 개선
- 이벤트 등장 여부를 함수로 분리
- 이벤트가 없을 경우 바로 다음 스테이지 버튼 표시
- 등장 확률을 Inspector에서 조정 가능

고급 카드 효과 시스템
- 카드가 하나의 효과만 가지는 것이 아닌 여러 효과를 동시에 가질 수 있도록 제작

신규 카드
- 연속 카드 (5데미지 2번공격)
- 흡혈 공격 (공격 + 회복)
- 광전사 (공격 + 자해)
- 철벽 (방어)
- 폭발 (공격 + 자해)

전투 로그 시스템
- 카드 사용 로그 출력
- 적 공격 로그 출력
- 방어도 로그 출력
- 보상 획득 로그 출력
- 최근 5개 로그 표시

UI 시스템
- 전체 카드 수 표시
- 덱 카드 수 표시
- 손패 수 표시
- 묘지 수 표시
- 스테이지 표시
- 적 공격력 표시
- 전투 로그 표시
- HP Bar
- 카드 설명 표시
- 카드 강화 선택 UI
- 카드 제거 UI
- 카드 제거 선택 UI
- 카드 설명에 희귀도 표시

카드 강화 시스템
- 카드 강화 기능 구현
- 강화 시 이름에 +표시
- 강화된 카드 효과 수치 증가
- CardInstance 기반 개별로 카드 강화
- CardData 원본 데이터와 실제 카드 상태 분리

강화 선택 시스템
- 랜덤 강화 제거
- 강화 가능한 카드 후보 표시
- 원하는 카드 선택 강화
- 이미 강화된 카드 제외
- 강화 후보 카드 랜덤

카드 희귀도 시스템
- 희귀도 종류 설정 (Common, Rare, Epic)
- 보상 카드 등장 시 희귀도 확률 적용

카드 구조 리팩토링
- CardData는 원본 데이터만 관리
- CardInstance는 실제 덱에 존재하는 카드 1장 관리
- 카드별 강화 상태 개별 저장
- 같은 종류의 카드라도 개별 강화

## 클래스 구조
BattleManager
- 전투진행
- 카드 사용
- 적 관리
- 보상 처리
- 스테이지 진행
- 카드 강화

CardData (ScriptableObject)
- 카드 이름
- 카드 타입
- 데미지
- 회복량
- 방어도
- 카드 설명 생성
- 강화 증가 수치
- 연속 공격 여부
- 공격 횟수
- 자해데미지

CardInstance
- CardData 참조
- 카드 강화 여부
- 강화 적용 후 데미지
- 강화 적용 후 회복량
- 강화 적용 후 방어도
- 카드 설명
- 희귀도 설정

EnemyData (ScriptableObject)
- 적 이름
- 최대 체력
- 공격력
- 보스 여부
- 특수 공격 데미지
- 특수 공격 주기

CardRarity Enum
- Common, Rare, Epic

## 사용기술
- Unity
- C#
- GitHub

## 오류목록
- Random 함수 유니티엔진, 시스템함수 충돌 오류 해결
- ScriptableObject로 변경 후 데이터타입 오류
- 강화 수치 중첩 문제

## 구현 예정
- 저장 기능
- UI 개선
- 카드 애니메이션
- 상점
- 코드 리팩토링
- 밸런스 테스트
- 잡몹 능력 추가

